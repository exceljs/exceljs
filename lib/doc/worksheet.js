...
const Worksheet = (module.exports = function(options) {
  options = options || {};
...
Worksheet.prototype = {
  ...
  set columns(value) {
    ...
    value.forEach(defn => {
      const column = new Column(self, count++, false);
      columns.push(column);
      column.defn = defn;
    });
  },
  ...
  getBackgroundImageId() {
    return this._media.filter(m => m.type === 'background').map(m => m.imageId)[0];
  },
