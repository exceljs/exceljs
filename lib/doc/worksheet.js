...
class Worksheet {
  ...
  duplicateRow(start, amount, insert = true) {
    const nEnd = this._rows.length;
    
    // insert new cells if it should
    if(insert) {
      for (let i = nEnd; i > start; i--) {
        const rSrc = this._rows[i - 1];
        if (rSrc) {
          const rDst = this.getRow(i + amount);
          ...
          this._rows[i + amount - 1] = undefined;
        }
      }
    }
    ...
    // account for defined names
    this.workbook.definedNames.spliceRows(this.name, start + 1, 0, amount);
  }
