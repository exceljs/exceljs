...
class Worksheet {
  ...
  addRow(value, styleOption = 'n') {
    ...
    row.values = value;
    this._setStyleOption(rowNo, styleOption[0] === 'i' ? styleOption : 'n');
    return row;
  }
  ...
  insertRows(pos, values, styleOption = 'n') {
    ...
    if (styleOption !== 'n') {
      ...
      for (let i = 0; i < values.length; i++) {
        if (styleOption[0] === 'o' && this.findRow(values.length + pos + i) !== undefined) {
          this._copyStyle(values.length + pos + i, pos + i, styleOption[1] === '+');
        } else if (styleOption[0] === 'i' && this.findRow(pos - 1) !== undefined) {
          this._copyStyle(pos - 1, pos + i, styleOption[1] === '+');
        }
      }
    }
  }
  ...
  _setStyleOption(pos, styleOption = 'n') {
    if (styleOption[0] === 'o' && this.findRow(pos + 1) !== undefined) {
      this._copyStyle(pos + 1, pos, styleOption[1] === '+');
    } else if (styleOption[0] === 'i' && this.findRow(pos - 1) !== undefined) {
      this._copyStyle(pos - 1, pos, styleOption[1] === '+');
    }
  }

  _copyStyle(src, dest, includeEmpty = false) {
    const rSrc = this.getRow(src);
    const rDst = this.getRow(dest);
    rDst.style = Object.freeze({...rSrc.style});
    // eslint-disable-next-line no-loop-func
    rSrc.eachCell({includeEmpty}, (cell, colNumber) => {
