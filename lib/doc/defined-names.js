...
DefinedNames.prototype = {
  ...
  getRanges: function(name, matrix) {
    ...
    if (!matrix) {
      return { name: name, ranges: [], formula: this.formulaMap[name] };
    }
    ...
    return {
      name: name, ranges: ranges, formula: this.formulaMap[name]
    };
  },
  ...
  get model() {
    ...
    return _.map(this.matrixMap, function (matrix, name) {
      return self.getRanges(name, matrix);
         ...
         * XXXCE no more filtering by length because we need to return formulae too
