...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    // Iterate the zip natively when possible: the unzip stream only emits the next
    // entry once the current one is drained, and native async iteration honours that
    // backpressure. The previous manual pause/resume helper could drop trailing
    // entries (e.g. sharedStrings.xml emitted after a worksheet that gets deferred).
    const zipEntries = zip[Symbol.asyncIterator] ? zip : iterateStream(zip);
