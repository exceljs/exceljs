...
class WorkbookReader extends EventEmitter {
  ...
  async read(input, options) {
    try {
      for await (const {eventType, value, hasMultipleValues} of this.parse(input, options)) {
  ...
  *_parseWorkbook(entry, options) {
    ...
    yield {eventType: 'workbook', value: this.properties.parseStream(entry)};
  }
  ...
  async *_parseSharedStrings(entry, options) {
    ...
    for await (const events of stream) {
      ...
      yield {eventType: 'shared-string', value: sharedStringEvents, hasMultipleValues: true};
    }
  }
  ...
  *_parseStyles(entry, options) {
    ...
    yield {eventType: 'styles', value: this.styles};
  }
