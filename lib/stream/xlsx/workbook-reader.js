...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    for await (const entry of iterateStream(zip)) {
        ...
        default:
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              // Worksheet arrived before shared strings (or workbook rels).
              // Buffer it in memory via iterateStream (the same draining the
              // parse paths use); parsing happens after the zip is fully read.
              // NB: using entry.pipe() + entry.autodrain() here previously raced
              // and could skip the following zip entry (e.g. sharedStrings).
              const chunks = [];
      ...
      // Drain anything the handlers above didn't consume (e.g. '_rels/.rels',
      // or parsers that stop early). For the in-memory buffered worksheet this
      // is a no-op since iterateStream already fully drained the entry.
      entry.autodrain();
    }
    ...
    for (const { sheetNo, data } of waitingWorkSheets) {
      ...
      stream.end(data);
      yield* this._parseWorksheet(stream, sheetNo);
    }
  }
