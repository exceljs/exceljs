...
const {PassThrough, Readable, pipeline} = require('readable-stream');
...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    const zip = pipeline(stream, unzip.Parse({forceStream: true}), (err) => {
      if (err) zip.emit('error', err);
    });
            ...
            if (this.sharedStrings && this.workbookRels) {
              ...
              await new Promise((resolve, reject) => {
                tmp.file((err, path, fd, tempFileCleanupCallback) => {
                  ...
                  pipeline(entry, fs.createWriteStream(path), (err) => {
