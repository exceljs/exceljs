...
class WorkbookReader extends EventEmitter {
  ...
  async *parse(input, options) {
    ...
    const streamEndedPromise = new Promise(resolve => {
      zip.on('close', () => {
        // close event is emitted twice for some reason
        ...
        default:
          if (entry.path.match(/xl\/worksheets\/sheet\d+[.]xml/)) {
            ...
            if (this.sharedStrings && this.workbookRels) {
              await this._parseWorksheet(iterateStream(entry), sheetNo);
            } else {
            ...
            sheetNo = match[1];
            await this._parseHyperlinks(iterateStream(entry), sheetNo);
          }
          break;
      }
      entry.autodrain();
