...
class BaseXform {
      ...
      for (const {eventType, value} of events) {
        if (eventType === 'opentag') {
          ...
          if (!this.parseClose(value.name)) {
            if (stream) stream.emit('finish'); // needed for unzipper to continue to the next entry
            return this.model;
          }
        }
      }
  ...
  async parseStream(stream) {
    return this.parse(parseSax(stream, ['opentag', 'text', 'closetag']), stream);
  }
