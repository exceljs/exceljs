...
class WorkSheetXform extends BaseXform {
  ...
  reconcile(model, options) {
    ...
    // Filter out any relationship whose target part wasn't loaded. Real-world
    // workbooks occasionally embed parts at non-canonical paths (e.g.
    // xl/sheet1_comments.xml instead of xl/commentsN.xml) that the loader's
    // filename regex doesn't recognise. Without filtering, lookups below
    // would dereference undefined. Doing this once here keeps every consumer
    // simple.
    const partsByRelType = {
      [RelType.Comments]: options.comments,
      [RelType.VmlDrawing]: options.vmlDrawings,
      [RelType.Table]: options.tables,
    };
    const relationships = (model.relationships || []).filter(rel => {
      if (rel.Type in partsByRelType) {
        return partsByRelType[rel.Type][rel.Target] !== undefined;
      }
      // Rel types not listed above (Hyperlink, Image, the drawing rel, …)
      // aren't subject to this filter; pass them through.
      return true;
    });

    const rels = relationships.reduce((h, rel) => {
      h[rel.Id] = rel;
    ...
    if (model.drawing) {
      ...
      if (match) {
        ...
        // The drawing part may not be in options.drawings if it parsed to a
        // null model (see _processDrawingEntry); skip media extraction then.
    ...
    // Any Table rel whose part wasn't loaded was already dropped from rels
    // above, so a tablePart pointing at one resolves to `undefined` here.
    model.tables = (model.tables || [])
      .map(tablePart => rels[tablePart.rId])
      .filter(Boolean)
      .map(rel => options.tables[rel.Target]);
