...
class VmlNoteXform extends BaseXform {
  ...
  normalizeAttributes(model, index) {
    ...
    if (model.note && model.note.insetmode === false) {
      delete vShapeAttributes['o:insetmode'];
    }
    return vShapeAttributes;
  }
    ...
    switch (node.name) {
      case this.tag:
        ...
        this.model = {
          anchor: [],
          note: {
            insetmode: (node.attributes['o:insetmode'] && true) || false,
          },
        };
        break;
      default:
